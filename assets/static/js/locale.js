...
// The raw country -> locale table, exported so the test suite can assert every
// entry (not just a hand-picked few) actually builds a working, localized
// formatter. main.js bundles locale.js and only imports the helpers it needs,
// so this extra export is tree-shaken out of the browser bundle.
export const COUNTRY_LOCALES = locales
...
// locales (no dayPeriod part), which the UI hides. periodFirst is true for
// locales that print the day period before the time (e.g. ko "오후 1:30",
// zh-Hant "下午1:30"), so the UI can reorder the marker rather than always
// trailing it.
export const formatTimeParts = (dateObj) => {
  ...
  // Keep the locale's own hour:minute separator by preserving the internal
  // literal (":" for most locales, "." for da/fi/id/kl/si). Only the day period
  // is dropped; trimming removes the now-dangling space literal that sat between
  // it and the time (leading for periodFirst locales, trailing otherwise).
  const time = parts
